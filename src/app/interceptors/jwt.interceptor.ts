...
export class JwtInterceptor implements HttpInterceptor {
      ...
      return next.handle(this.addToken(request)).pipe(
        catchError(err => {
          if (err instanceof HttpErrorResponse) {
            switch (err.status) {
              case 400:
                return throwError(err); // this.handle400Error(err);
              case 401:
  ...
  private static isInBlockedList(url: string): Boolean {
    return url == `${environment.api_url}/auth/jwt/create/` ||
      url == `${environment.api_url}/auth/jwt/refresh/` ||
      url == `${environment.api_url}/auth/users/reset_password_confirm/` ||
      url == `${environment.api_url}/auth/users/activation/` ||
      url == `${environment.api_url}/auth/users/reset_password/` ||
      url == `${environment.api_url}/auth/users/`;
  }
    ...
    if (this.authService.currentAccessToken) {
      return req.clone({
        headers: new HttpHeaders({
          Authorization: `JWT ${this.authService.currentAccessToken}`
        })
      });
    } else {
      return req;
      ...
      return this.authService.getNewAccessToken().pipe(
        switchMap((tokens: {access, refresh}) => {
          if (tokens) {
            ...
            return this.authService.storeAccessTokens(tokens).pipe(
              switchMap(_ => {
